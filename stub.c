...
// Allocate a memory region that can be written while code is generated. The
// MoonBit side switches it to RX during finalization.
uint64_t asm_dot_mbt_mmap(size_t length) {
    void* pointer = mmap(NULL, length, PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE, -1, 0);
    return (uint64_t)(uintptr_t)pointer;
}

int32_t asm_dot_mbt_mmap_failed(uint64_t pointer) {
    return (void*)(uintptr_t)pointer == MAP_FAILED;
}

int32_t asm_dot_mbt_errno() {
    return errno;
}

int32_t asm_dot_mbt_mprotect_rx(uint64_t start, size_t length) {
    return mprotect((void*)(uintptr_t)start, length, PROT_READ | PROT_EXEC);
}
...
// Copy generated bytes into the writable page.
void asm_dot_mbt_copy_code(uint64_t start, const void* machcode, size_t length) {
    memcpy((void*)start, machcode, length);
}

typedef int64_t (*asm_dot_mbt_i64_fn_t)(void);

asm_dot_mbt_i64_fn_t asm_dot_mbt_func_ref_i64(uint64_t start) {
    return (asm_dot_mbt_i64_fn_t)(uintptr_t)start;
}
